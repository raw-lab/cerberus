...
"""pipeline.py Cerberus module for processing the main pipeline jobs.
"""
...
def run_jobs(fastq, fasta, amino, rollup, config, outpath):
	...
	if fastq:
		...
		for key,value in fastqPaired.items():
			...
			pipeline[cerberus_qc.checkQuality.remote([forward,reverse], config['EXE_FASTQC'],
			...
			if not config['EXE_FASTP']:
				...
				r1,r2 = cerberus_trim.trimPairedRead([key, [value,reverse]], config, Path(outpath, STEP[3], key))
			value = cerberus_merge.mergePairedEnd([r1,r2], config, Path(outpath, STEP[3], key, "merged"))
		...
		for key,value in fastq.items():
			pipeline[cerberus_qc.checkQuality.remote(value, config['EXE_FASTQC'], Path(outpath, STEP[2], key))] = key
		# Trim
		if config['NANOPORE'] and not config['EXE_PORECHOP']:
			...
			for key,value in fastq.items():
				pipeline[cerberus_trim.trimSingleRead.remote([key, value], config, Path(outpath, STEP[3], key))] = key
	...
	if fasta:# and "scaffold" in config:
		if config['REMOVE_N_REPEATS']:
			...
			for key,value in fasta.items():
				pipeline[cerberus_formatFasta.removeN.remote(value, Path(outpath, STEP[5], key), config['REPLACE'])] = key
		else:
			for key,value in fasta.items():
	...
	if rollup:
		...
		for key,value in rollup.items():
			...
			for hmm in dbHMM:
				...
				pipeline[cerberus_hmm.filterHMM.remote(value, tsv_filtered, dbHMM[hmm], config['REPLACE'])] = f"{hmm}/{key}"
	...
	while pipeline:
		...
		if func == "trimSingleRead":
			...
			fastq[key] = value
			pipeline[cerberus_qc.checkQuality.remote(value, config['EXE_FASTQC'], Path(outpath, STEP[3], key, "quality"))] = key+'_trim'
			if fastq and config['ILLUMINA']:
				if config['SKIP_DECON'] or not config['EXE_BBDUK']:
					...
					pipeline[cerberus_formatFasta.reformat.remote(value, Path(outpath, STEP[5], key), config['REPLACE'])] = key
				else:
					set_add(step_curr, 4, "STEP 4: Decontaminating trimmed files")
					pipeline[cerberus_decon.deconSingleReads.remote([key, value], config, Path(outpath, STEP[4], key))] = key
			else:
				set_add(step_curr, 5.2, "STEP 5b: Reformating FASTQ files to FASTA format")
				pipeline[cerberus_formatFasta.reformat.remote(value, Path(outpath, STEP[5], key), config['REPLACE'])] = key
		if func.startswith("decon"):
			fastq[key] = value
			set_add(step_curr, 5.2, "STEP 5b: Reformating FASTQ files to FASTA format")
			pipeline[cerberus_qc.checkQuality.remote(value, config['EXE_FASTQC'], Path(outpath, STEP[4], key, "quality"))] = key+'_decon'
			pipeline[cerberus_formatFasta.reformat.remote(value, Path(outpath, STEP[5], key))] = key
		if func == "removeN" or func == "reformat":
			if func == "removeN":
				...
				readStats[key] = cerberus_metastats.getReadStats(value[0], config, Path(outpath, STEP[6], key))
			elif func == "reformat":
				fasta[key] = value
			set_add(step_curr, 7, "STEP 7: ORF Finder")
			if key.startswith("FragGeneScan_"):
				pipeline[cerberus_genecall.findORF_fgs.remote(fasta[key], config, Path(outpath, STEP[7], key))] = key
			elif key.startswith("prodigalgv_"):
				pipeline[cerberus_genecall.findORF_prod.remote(fasta[key], config, Path(outpath, STEP[7], key), config['META'], True)] = key
			elif key.startswith("prodigal_"):
				pipeline[cerberus_genecall.findORF_prod.remote(fasta[key], config, Path(outpath, STEP[7], key), config['META'])] = key
			elif key.startswith("phanotate_"):
				pipeline[cerberus_genecall.findORF_phanotate.remote(fasta[key], config, Path(outpath, STEP[7], key), config['META'])] = key
			groupORF += 1
			...
			if config['CHUNKER'] > 0:
				...
				for hmm in dbHMM.items():
					...
					for chunk in chunks:
						...
						chunkCount += 1
						#TODO:Fix this after update to hmmSearch
...
#			for key,tsv_file in zip(keys,value):
		...
		if func.startswith('filterHMM'):
			...
			pipeline[cerberus_parser.parseHmmer.remote(value, config, Path(outpath, STEP[9], key), hmm, dbHMM[hmm])] = key
			...
			if len(hmm_tsvs[key]) == len(dbHMM):
				...
				pipeline[cerberus_parser.top5.remote(tsv_filtered, outfile)] = key
		if func.startswith('parseHmmer'):
			if not value:
				print("WARNING: no hmm results for:", key)
			if key not in hmmRollup:
				hmmRollup[key] = dict()
			hmmRollup[key].update(value)
			pipeline[cerberus_parser.createCountTables.remote(value, Path(outpath, STEP[9], key), config['REPLACE'])] = key
...
def report(outpath, config, dbHMM, fasta, amino, hmm_tsv, hmm_tsvs, hmmRollup, hmmCounts, readStats, NStats):
	...
	for key in hmm_tsvs.keys():
		...
		jobStat[cerberus_prostats.getStats.remote(amino[key], hmm_tsvs[key], hmmCounts[key], config['MINSCORE'], dbHMM, summary_tsv, Path(final_path, "fasta", f"{key}.faa"))] = key
	while jobStat:
	...
	for key in hmm_tsvs.keys():
		...
		if len(gff) == 1:
			...
			jobStat[cerberus_report.write_datafiles.remote(gff[0], fasta[key], amino[key], summary_tsv, out_gff, out_genbank)] = None
		else:
			out_gff = Path(final_path, "gff", f"{key}.gff")
	...
	if config['SKIP_PCA']:
		...
		print("PCA Analysis")
		pcaFigures = cerberus_visual.graphPCA.remote(dfCounts)
	...
	for key,value in hmmRollup.items():
		jobCharts[cerberus_visual.graphBarcharts.remote(value, hmmCounts[key])] = key
	...
	cerberus_report.createReport(figSunburst, figCharts, Path(outpath, STEP[10]))
